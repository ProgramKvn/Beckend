...
//topicss Model
const Topicss = require("../../models/topicCss");

//@routers GET api/topics
//@desc  Get all TOPIC
router.get("/", async (req, res) => {
  try {
    const topics = await Topicss.find();
...
router.get("/:id", async (req, res) => {
  try {
    const topic = await Topicss.findById(req.params.id);
...
router.post("/", async (req, res) => {
  const newTopic = new Topicss(req.body);
  try {
...
router.delete("/:id", async (req, res) => {
  try {
    const topic = await Topicss.findByIdAndDelete(req.params.id);
...
router.patch("/:id", async (req, res) => {
  try {
    const topic = await Topicss.findByIdAndUpdate(req.params.id, req.body);
